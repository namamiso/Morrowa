...
- 現時点の主要仕様書は `docs/Morrowa_仕様書_v0.1.md` に置く。
- Morrowa の画面構成、ToDo、Habit、アラーム、通知、ランチャー独自仕様、MVP 範囲はこの仕様書を正本として扱う。
- 要件や仕様を変更する場合は、該当する設計文書を更新し、必要に応じてこの `AGENTS.md` の要約も同期する。
- `AGENTS.md` は作業者向けの案内板であり、詳細仕様の完全な代替ではない。
