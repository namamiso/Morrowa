...
## 13. 2026-07-09〜07-15 の記録(v1 全滅 → ロールバック → v2 完成 → 設定/余白修正 → 残項目 A・B 全消化)

本章は §12.7(2026-07-05)以降の正史。各トピックの正本は以下で、本章は総覧と「他文書に書いていない詳細」を残す。

- ドロワー編集 v1 の失敗分析・ロールバック決定: `Morrowa_AppDrawer_編集モード_実装計画.md` §10.39〜10.42
- ドロワー編集 v2 の要件・設計・受け入れ記録: `Morrowa_AppDrawer_編集モードv2_要件設計.md`
- ホーム下部余白(inset 二重計上): `prds/remove-hotseat-qsb.md`(2026-07-14 改訂)
- 残項目の全量と状態: `Morrowa_残項目一覧.md`

### 13.1 経緯総覧(時系列)

| 日付 | 出来事 | 主要コミット |
|---|---|---|
| 07-09〜10 | ドロワー編集 v1(タスクG/J)を worktree 並行実装・静的レビュー+コンパイルのみで統合 → **実機で全滅**(§10.40) | `f69fdba` `57f0f7a` |
| 07-12 | 真因究明(§10.41): J1系4件+G系4件を静的解析で特定、J1系を修正 → それでも実機挙動不良 | `f13a89d` |
| 07-12 | **戦略判断**: イタチごっこ中止。フォーク元変更案も検討し「敗因はホスト内部を曲げる実装方針」と結論、Lawnchair 残留のまま**編集モード全ロールバック**(§10.42、安定点 `823711035f` へ)。5原則(Morrowa 所有レイヤー/標準ドラッグ/一方向状態/テスト先行/実機関門)を制定 | `a35bd35` |
| 07-12〜14 | v2 を質問形式で要件確定 → P0(純Kotlinモデル+テスト)→ P1(DrawerOrder v4+読みwalk)→ P2(編集オーバーレイ+ItemTouchHelper)→ P3(複数選択フォルダ操作)→ P4(フォルダ編集サブビュー)を各フェーズ実機受け入れで完走 | `659fc4b` `853de30` `8ffaea1` `8f6f2e7` `abba900` |
| 07-14 | P4 実機バグ「フォルダ内編集が完了後に画面へ反映されない」→ AOSP `AdapterItem` の比較素通しが原因(v2 唯一の AOSP 変更として修正) | `0a6776857c` |
| 07-14 | ドロワー列数が常に4列 → **ANR 修正コミット `bb6f9e07fb` の regression** と特定し修正(§13.3) | `c5460eb` |
| 07-14 | ホーム下部の謎の余白 → **inset 二重計上**と特定し修正(§13.4) | `292f335` |
| 07-15 | 残項目の全量棚卸し → A(3件)クローズ、B(4件)全解決(§13.5) | `30eb625` `5f426a5` `85ae098` |

### 13.2 達成済み一覧(2026-07-15 時点)

**MVP ゴール(実装指示書 §2)** — 全項目実装済み:
4論理ページ / Home・Widget Blank 通常ページ / Habit・ToDo 専用固定画面 / 横スワイプ循環 / 初回 Home・最終ページ復元 / Habit 保存+草カレンダー / ToDo 保存 / アラーム+未達通知(SCHEDULE_EXACT_ALARM+setWindow フォールバック)/ ゴミ箱30日 / JSON バックアップ / Dock・QSB 除去+下部グリッド統合。§11 未確定事項5件も実装内容で確定済み(指示書 §11 に記載)。

**ドロワー編集 v2(要望A〜D 相当)** — P0〜P4 全フェーズ実機受け入れ済み。AOSP 変更は AdapterItem 比較の1件のみ、単体テスト51件。

**品質修正(本章で詳細)** — 設定デフォルト固定バグ 12+11 箇所(§13.3)、下部余白(§13.4)、Dock migration、hideFolderApps=false エッジ。

### 13.3 【詳細】設定が全てデフォルト固定になっていたバグ(bb6f9e07fb regression)

**発覚**: 「ドロワー列数を変えても常に4列」。ユーザーは「フォーク前からのバグ」と推測し本家イシューも調査したが該当なし — **本フォーク固有**だった。

**真因**: 2026-06-19 の ANR 修正 `bb6f9e07fb` が、起動パスの `firstBlocking()`(保存値を読む・ブロッキング)を `.defaultValue`(静的デフォルトを返すだけ)へ一括置換。コミットメッセージは「defaultValue + Flow-based async update」と謳ったが、**追い適用側が実装されないまま**だったため、置換された全設定が永久にデフォルト固定になった。設定 UI 自体は DataStore を直接読むため「変更できたように見えるのに効かない」症状になる。

**第1弾修正(`c5460eb`)**: `DeviceProfileOverrides` — 列数のほか、フォルダ列数・ホーム/ドロワーアイコンサイズ係数・ラベル設定・taskbar-on-phone が対象だった。`IdpPreference.storedValue()`(グリッド非依存の保存値 Flow)を追加し、combine した Flow を `@Volatile` キャッシュへ。初回 initGrid はデフォルトで即描画(ANR ゼロ維持)、保存値到着時に差があれば1回だけ reloadGrid。**トレードオフ**: 起動直後の一瞬だけデフォルト表示 → ユーザー了承済みの仕様。おまけで `close()` の `TODO()`(呼ばれたら即クラッシュ)も解消。

**第2弾修正(`5f426a5`)**: 残り14箇所をトリアージした結果:

| 箇所 | 設定 | 判定 |
|---|---|---|
| LawnchairThemeManager ×2 | アイコン形状・フォルダ形状 | **バグ**(オブザーバは発火するのに再計算がデフォルトを読む) |
| LawnchairUtils ×3 | フォルダ背景/プレビュー不透明度・ドロワー背景色 | **バグ**(毎描画で読むため永久に無効) |
| AllAppsSearchInput ×2 | 検索バーのマイク/レンズ表示・検索バー非表示 | **バグ** |
| IconShapeManager | ウィンドウ遷移の角丸 | **バグ** |
| DrawableTokens | Work タブ背景色 | **バグ** |
| LawnQsbLayout | QSB 検索プロバイダ(全呼び出し元に影響) | **バグ** |
| LawnchairSearchAlgorithm | ドロワー検索エンジン選択 | **バグ** |
| ThemeProvider ×2 | アクセント色・カラースタイル | 軽微(既存オブザーバが数フレームで自己修正) |
| OverlayCallbackImpl | フィード ON/OFF | 軽微(LawnchairLauncher 側のオブザーバが修正) |

修正は共通基盤 **`PreferenceCaches`**(`lawnchair/src/app/lawnchair/preferences2/PreferenceCaches.kt`)を新設: 各値はデフォルトで開始 → DataStore Flow で非同期補正(`LawnchairApp.onCreate` でウォームアップ開始、UI inflate 前に温まる)→ 以後の変更も追従。メインスレッドブロックは一切なし。

**再発防止ルール(重要)**: 同期パス(描画・inflate・初期化)で preference の現在値が要る場合、`firstBlocking` も `.defaultValue` も使わず **`PreferenceCaches` に登録して `.value` を読む**。IDP 系(グリッド依存デフォルト)は `IdpPreference.storedValue()` + `DeviceProfileOverrides` のキャッシュ方式に倣う。

### 13.4 【詳細】ホーム下部余白(inset 二重計上)の診断記録

**経緯**: Galaxy 実機でホーム最下段の下に「1行も入らない高さの謎の帯」。初回診断は「ナビゲーション inset そのもの(仕様)」としたが、ユーザーの観察「**ジェスチャー領域はさらにその下に既にある**」で誤診と判明 — 実機の見た目の症状が最短の切り分け情報だった(§10.41 の教訓の再演)。スクリーンショット2枚(Morrowa vs Yahoo きせかえ)で帯の高さ≒ナビバー高さを確認し確定。

**真因**: グリッド高さの計算元 `availableHeightPx`(`WindowBounds.availableSize` = 画面高さ−inset)が既に下端 inset を除外しているのに、hotseat 無効分岐が `workspacePadding.bottom = mInsets.bottom` を追加確保(`7a9ee72639`)→ **inset が2回差し引かれ**、ナビ領域の上に inset 分の死に帯。AOSP の hotseat 有効分岐が `−mInsets.bottom` している理由がまさにこの二重計上の打ち消し。経緯としては、Phase 8 当初の `0−mInsets.bottom`(負値→グリッドがナビ下に潜る事故)を直す際に 0 を通り越して +inset にした過剰修正だった。

**修正(`292f335`)**: 無効分岐を `paddingBottom = 0` に。最下段はナビ/ジェスチャー領域の直上まで届き、潜り込みは availableHeightPx 側の確保で防止。PRD の「mInsets.bottom は除去しない」は「二重計上を除去(1回分は維持)」に改訂済み。実機受け入れ済み。

### 13.5 残項目の現状(正本: `Morrowa_残項目一覧.md`)

- **A(MVP 完了に必要)**: 3件全てクローズ。A-1「ドロワー→ホーム配置が無い」は統括側の誤認で、ロールバックにより v1 のゲートも消え OSS 標準のドラッグ配置が健在(実機確認済み)。
- **B(バグ・負債)**: 4件全て解決(B-1 上記 / B-2 `MorrowaDockMigration`: bind 後 one-shot で旧 Dock アイテムを空きセル下行優先で移動、実機は Dock アイテム無しのため実質 no-op 確認のみ / B-3 hideFolderApps=false の重複キー: スナップショット除外+`DrawerOrderMerge.memberOf` でメンバーをフォルダ直後に配置 / B-4 v1 残骸監査: クリーン)。B-1/B-2/B-3 は実機確認待ち。
- **C(ドロワー編集第2フェーズ・任意)**: C-1 ドラッグ重ね合体(中規模・実機チューニング必須)/ C-2 入口追加(小)/ C-3 編集モードからのホーム配置(必要性低)。
- **D(スコープ外15件・要仕様改訂)**: 着手するなら ToDo/Habit 拡張(タグ・サブタスク・完了履歴・統計)が費用対効果最良 → カレンダー系 → 同期・共有系(最重量)の順。
- **E(運用)**: E-1 リリースビルド整備(署名・R8 検証)未着手 / E-2 本章の追記により**完了** / E-3 CLAUDE.md の戦略更新 未着手。

### 13.6 教訓・運用ノート(他文書未記載)

1. **v2 の5原則は実証された**: v1 全滅と同じ機能セットを、AOSP 変更1件・レース源ゼロ・テスト51件で完成。実機バグも2件とも「データ層は正常、ビュー層の1点」と数分で切り分けられる構造だった。
2. **ロールバック後の stale 知識に注意**: v2 P3 で、ロールバックで消えた v1 の DAO ヘルパ(`createFolderWithItems`)を「存在する」と誤認して参照しコンパイルを壊した。ロールバック直後の実装では、参照 API を現ツリーに対して機械的に実在確認する。
3. **診断はコードより先に実機の観察**: 余白問題・G/J 全滅とも、ユーザーの「見た目の症状」が静的解析の誤りを最短で正した。
4. **ビルド/テスト環境**: ビルド・単体テスト・実機(Galaxy `R5CT3378XTJ`)はユーザーの **Mac**(`./gradlew`。過去文書の `.\gradlew.bat` 記述は旧環境)。統括側の Linux 作業機には JDK/adb が無く、コンパイル確認はコミット+push して依頼する運用。
5. **git 運用**: 作業機の identity はリポジトリローカルに `namamiso <hanpenneko@gmail.com>` を設定済み。ネットワーク断で push が長時間失敗した事例があり、その際はバックグラウンドの until ループ(2分間隔)でリトライして自動復旧した。
6. **Room スキーマ**: 現行 v4(`DrawerOrder` 追加)。v1 期の v4/v5 とは別物。スキーマ JSON はビルド時に生成されるため、DB 変更後の初回ビルド後に `schemas/` をコミットする。
