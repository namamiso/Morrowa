...
interface HabitDao {
    ...
    @Query(
        "UPDATE habit_rules SET endDate = :endDate WHERE endDate IS NULL " +
            "AND habitId IN (SELECT id FROM habits WHERE isArchived = 1)",
    )
    suspend fun closeOpenRulesForArchivedHabits(endDate: String)
